...
- コード内コメント・JSDoc・テストの`describe`/`it`名など、コードに含まれるあらゆるテキストに
  「〇〇からの指示」「Issue #XXで発覚/指摘/対応」のような由来・経緯(いつ・誰の指示で・どのIssueで)が
  新規に書き込まれていないか。`(#123)`のようにIssue番号だけを括弧書きで添えるパターンも同様に対象
  (「Issue」という単語の有無は問わない)。**このパターンを1件でも見つけたら、他の観点の良し悪しに
  関わらず必ず要修正判定にすること**(由来・経緯はコミットメッセージ・PR本文側に残せば十分)
- コメント・JSDocが「何のためか」「パフォーマンス・セキュリティ等の観点でなぜこの処理をここに書くか」
  ではなく、「〇〇という対応をした結果△△という問題が起こるため」のような背景シナリオを丁寧になぞる
  冗長な説明になっていないか
