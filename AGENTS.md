...
## GitHub Projects 運用
- 作業を開始したら、以下のコマンドでその Issue の Project ステータスを更新すること
  (PROJECT_NUMBER, ORG/USER, ITEM_ID は Issue から `gh project item-list` 等で取得)
  ```bash
  gh project item-edit --id <ITEM_ID> --field-id <STATUS_FIELD_ID> \
    --project-id <PROJECT_ID> --single-select-option-id <TODO_OPTION_ID>
  ```
- PR作成後は Status を `In Review` に更新する
...
- reviewer が LGTM を出さなかった場合は絶対にマージしない。Projectsのステータスを `In Review` のままにし、人間の判断を待つ
