...
- コメントは要否を判断してから書く。変数代入・単純な条件分岐・フレームワーク規約通りの記述など、
  コードを読めば自明な処理にはコメントを書かない。一目では理解しづらい複雑なロジック(非自明な
  アルゴリズム、ライブラリの制約への対処、パフォーマンス上の理由による特殊な実装など)にのみ
  コメントを許可し、その場合も核心を突いた1〜2行程度に収める
- コード内コメント・JSDoc・テストの`describe`/`it`名など、コードに含まれるあらゆるテキストに
  ...
  PR本文(`Closes #XX` 等)を辿れば十分な情報であり、コード側に残すのはリファクタリングやIssueクローズ後に
  文脈が失われて陳腐化するだけの純粋なノイズ。reviewerはこのパターンを見つけたら他の観点の良し悪しに
  関わらず必ず要修正判定にする(機械的なreject対象)
- コメント・JSDocは「何のためにこの処理があるか」「パフォーマンスやセキュリティなど、なぜこの処理を
  ここに書く必要があるか」だけを端的に書く。「〇〇という対応をした結果△△という問題が起こるため」の
  ように背景シナリオを丁寧になぞる説明は書かない。背景説明がどうしても必要な場合も、結論(何のため・
  なぜここか)を先に一言で述べ、背景は補足として最小限に留める
